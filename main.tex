...
\end{equation}
RAA incorporates an ERA-based penalty for underperformance relative to FIP:
\begin{equation}
\mvar{RAA} = \frac{(\mvar{AvgRuns} - \mvar{FIP} - \mvar{ERA} \cdot C_{\text{era}} \cdot k_{\text{adj}}) \cdot \mvar{IP}}{6 \cdot \max(\mvar{ERA} + \mvar{WHIP}, 1.5)}
\end{equation}
Where, $k_{\text{adj}}$ serves as a penalty coefficient, set to 3.0 when $\mvar{ERA} \ge 3.0$ and 1.0 otherwise.
