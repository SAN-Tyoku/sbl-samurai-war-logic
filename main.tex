...
Similarly, for cases where $\mvar{UZR}_{\text{basic}} > T_{\text{high}}$, positive outliers are compressed:
\begin{equation}
\mvar{UZR} = T_{\text{high}} + \frac{\ln(\mvar{UZR}_{\text{basic}} - T_{\text{high}} + 1)}{\ln \beta}
\end{equation}
where $T_{\text{high}}$ is the upper bound threshold (e.g., $1.2$). This formulation ensures that elite defensive impact does not disproportionately skew the overall WAR distribution beyond the specified limit.
